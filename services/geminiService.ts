...
/**
 * Helper function to call Gemini API with retry logic for 429 errors
 */
...
export const getTopicSuggestions = async (config: Partial<TestConfig>): Promise<string[]> => {
  ...
  const response = await callGeminiWithRetry(() => ai.models.generateContent({
    model: 'gemini-3-flash-preview', // Sử dụng Flash cho gợi ý nhanh
    contents: prompt,
    config: {
...
export const generateTest = async (config: TestConfig): Promise<GeneratedTest> => {
  ...
  const response = await callGeminiWithRetry(() => ai.models.generateContent({
    model: 'gemini-3-flash-preview', // Switch to Flash to avoid 2 RPM limit of Pro model
    contents: prompt,
    config: {
